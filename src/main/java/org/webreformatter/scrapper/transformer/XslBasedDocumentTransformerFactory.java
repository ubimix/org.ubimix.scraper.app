...
public class XslBasedDocumentTransformerFactory {
    ...
    public static XmlWrapper getXslDocument(
            ...
            IProtocolHandler protocolHandler, Uri xslUrl)
        ...
        HttpStatusCode status = protocolHandler.handleRequest(xslUrl,
                login, password, resource);
    ...
    public XslBasedDocumentTransformer getTransformer(Uri xslUri)
            throws IOException, XmlException {
        XmlWrapper xsl = getXslDocument(fResourceProvider,
                fProtocolHandler, xslUri);
